...
type PassportData = {
  ...
  scouting: { label: string; value: number; detail: string; score: number; source: string }[];
  playstyles: string[];
...
function getTechData(name: string) {
  ...
  const exactMatch = Object.keys(LANG_MAP).find(k => k.toLowerCase() === lowerName);
  if (exactMatch) return LANG_MAP[exactMatch];

  const partialMatch = Object.keys(LANG_MAP).find(k => k.length > 2 && lowerName.includes(k.toLowerCase()));
    ...
    >
      ...
      <div className="px-6 pt-6 pb-5 flex items-center gap-4 bg-[#0d1524] border-b border-white/5 relative z-10">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[#a1b0cb] text-[10px] font-mono mt-0.5 truncate">@{data.user.login}</div>
          {data.user.url && (
    ...
    <div className="flex flex-col h-full z-10">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="px-3 py-1 rounded border border-[#e9b646]/30 bg-[#e9b646]/10 flex items-center gap-2">
           ...
           <span className="text-white text-[10px] font-semibold uppercase">{data.level.title}</span>
        </div>
        <div className="flex items-center gap-2">
           <span className="text-[#a1b0cb] text-[8px] font-mono uppercase tracking-wider">Primary</span>
           <div className="flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded border border-white/10">
              {React.createElement(getTechData(primaryLang).icon, { className: "w-3 h-3", style: { color: getTechData(primaryLang).color } })}
              <span className="text-white font-bold text-[9px] uppercase tracking-wide">{primaryLang}</span>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-2 mb-3 flex-1 content-start">
         {data.scouting.slice(0, 8).map((metric) => {
            ...
            return (
              <div key={metric.label} className="flex flex-col p-2 bg-white/[0.03] rounded-lg border border-white/5 shadow-inner">
                 <div className="text-[#a1b0cb]/80 text-[8px] font-mono uppercase tracking-widest mb-0.5 truncate">{metric.label}</div>
                 <div className="flex items-baseline gap-1.5 overflow-hidden">
                    <span className="text-white font-bold text-[15px] leading-none truncate">{num}</span>
                    <span className="text-[#e9b646] text-[7px] font-mono uppercase tracking-wider truncate">{text}</span>
                 </div>
              </div>
            );
         })}
      ...
      <div className="mt-auto bg-black/20 rounded-lg p-2.5 border border-white/5">
         ...
         <div className="flex flex-wrap gap-1.5">
            {data.playstyles.slice(0, 3).map((style) => (
              <div key={style} className="flex items-center gap-1.5 bg-white/5 px-2 py-1 rounded-md border border-white/5">
                 <div className="w-1 h-1 rounded-full bg-[#e9b646]" />
                 <span className="text-slate-200 text-[8.5px] font-medium uppercase tracking-wide">{style}</span>
              </div>
            ))}
         </div>
      </div>
  ...
  const renderSection = (title: string, tools: string[]) => {
    ...
    return (
       <div className="mb-6 flex flex-col items-center">
          <div className="text-[#c09a5c] text-[10px] font-bold tracking-wider uppercase mb-3 text-center">{title}</div>
          <div className="flex justify-center gap-6">
             {tools.slice(0, 4).map(tool => {
                const tech = getTechData(tool);
                return (
                   <div key={tool} className="flex flex-col items-center">
                      {React.createElement(tech.icon, { className: "w-8 h-8 mb-2", style: { color: tech.color, filter: `drop-shadow(0 0 5px ${tech.color}90)` } })}
                      <span className="text-white text-[9px] font-medium text-center">{tool}</span>
                   </div>
                );
             })}
          </div>
    ...
    <>
      ...
      <div className="flex-1 flex flex-col justify-center">
         {data.stack && data.stack.length > 0 ? (
            data.stack.slice(0, 4).map((section, idx) => (
       ...
       <div className="flex justify-between items-center px-4">
          <div className="flex items-center gap-2">
              <div className="p-1.5 bg-[#1b2944] rounded">
                 {React.createElement(getTechData(primaryLang).icon, { className: "w-4 h-4", style: { color: getTechData(primaryLang).color } })}
              </div>
